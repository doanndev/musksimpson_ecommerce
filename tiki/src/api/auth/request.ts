...
export const enable2FARequest = async () => {
  const response = await instance.post('/auth/enable-2fa');
  return getSuccessResponse<string>(response);
};

export const verify2FARequest = async (data: TwoFactorRequest) => {
  const response = await instance.post('/auth/verify-2fa', data);
  return getSuccessResponse<string>(response);
};
